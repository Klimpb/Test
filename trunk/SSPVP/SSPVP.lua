...
function SSPVP:AutoJoinBattlefield()
	...
	if( SSPVP.db.profile.priority[ currentType ] <= SSPVP.db.profile.priority[ joinAbbrev ] ) then
...
function SSPVP:QueueReady( id, map )
	...
	elseif( joiningBF ~= id ) then
		...
		if( self.db.profile.priority[ SSPVP:GetBattlefieldAbbrev( map ) ] < self.db.profile.priority[ SSPVP:GetBattlefieldAbbrev( joinMap ) ] ) then
			joiningBF = id;
