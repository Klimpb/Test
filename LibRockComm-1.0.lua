...
--***************************************
	...
	L["Send out talent information to those who request it. In turn, you will be able to request talent info from others. This is equivalent to Armory, except in-game."] = "发送天赋信息给需要它的人。你将能向他人请求天赋信息。这个和Armory相同， except in-game."
...
local RockComm_QueryAddonVersion = RockComm.QueryAddonVersion
sanityChecks[function()
	return RockComm_QueryAddonVersion == RockComm.QueryAddonVersion
end] = true
...
local RockComm_AddAddonVersionReceptor = RockComm.AddAddonVersionReceptor
sanityChecks[function()
	return RockComm_AddAddonVersionReceptor == RockComm.AddAddonVersionReceptor
end] = true
...
local RockComm_QueryTalents = RockComm.QueryTalents
sanityChecks[function()
	return RockComm_QueryTalents == RockComm.QueryTalents
end] = true
...
local RockComm_AddTalentReceptor = RockComm.AddTalentReceptor
sanityChecks[function()
	return RockComm_AddTalentReceptor == RockComm.AddTalentReceptor
end] = true
...
do
	...
	local addonVersionPinger_OnCommReceive_PING = addonVersionPinger.OnCommReceive.PING
	local addonVersionPinger_OnCommReceive_PONG = addonVersionPinger.OnCommReceive.PONG
	sanityChecks[function()
		return type(addonVersionPinger) == "table" and type(addonVersionPinger.OnCommReceive) == "table" and addonVersionPinger_OnCommReceive_PING == addonVersionPinger.OnCommReceive.PING
	end] = true
	sanityChecks[function()
		return type(addonVersionPinger) == "table" and type(addonVersionPinger.OnCommReceive) == "table" and addonVersionPinger_OnCommReceive_PONG == addonVersionPinger.OnCommReceive.PONG
	end] = true
...
do
	...
	local talentPinger_OnCommReceive_PING = talentPinger.OnCommReceive.PING
	local talentPinger_OnCommReceive_PONG = talentPinger.OnCommReceive.PONG
	sanityChecks[function()
		return type(talentPinger) == "table" and type(talentPinger.OnCommReceive) == "table" and talentPinger_OnCommReceive_PING == talentPinger.OnCommReceive.PING
	end] = true
	sanityChecks[function()
		return type(talentPinger) == "table" and type(talentPinger.OnCommReceive) == "table" and talentPinger_OnCommReceive_PONG == talentPinger.OnCommReceive.PONG
	end] = true
